document.addEventListener('DOMContentLoaded', () => {
  ...
  // Trigger popup lewat klik logo (bukan welcomeMessage lagi)document.addEventListener('DOMContentLoaded', () => {
  // Redirect to login if no username is stored
  const username = localStorage.getItem('username');
  const isLoginPage = window.location.pathname.includes('login.html');
  if (!username && !isLoginPage) {
  ...
  if (username && welcomeMessage) {
    welcomeMessage.textContent = `Hallo ${username}!`;
    // popup ganti nama lewat klik nama, bukan logo
  ...
  // Klik logo juga toggle hamburger
  const navLogo = document.querySelector('.nav-logo');
  if (navLogo && menuToggle) {
  ...
  if (feedbackForm) {
    const botToken = '7857009719:AAFeqQ9fpy7FszBltsNSjqmrLn-QesALLh8';
